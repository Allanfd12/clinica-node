...
class Consulta {
    ...
    static async find(query) {
        var consultas = await global.connection.query(`SELECT c.*, p.nome AS nome_paciente, m.nome AS nome_medico, case when c.data < curdate() then 'realizada' when c.data = curdate() then 'hoje' else 'agendada' end as classe FROM consultas c INNER JOIN pacientes p ON c.id_paciente = p.id INNER JOIN medicos m ON c.id_medico = m.id WHERE p.nome LIKE '%${query}%' or m.nome LIKE '%${query}%' ORDER BY c.data DESC, c.hora DESC`);
        return consultas[0];
    }
